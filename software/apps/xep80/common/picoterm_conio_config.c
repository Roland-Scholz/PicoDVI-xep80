...
picoterm_conio_config_t conio_config = {.rvs = false, .blk = false, .just_wrapped = false, .wrap_text = true, .dec_mode = DEC_MODE_NONE, .cursor.pos.x = 0, .cursor.pos.y = 0, .cursor.state.visible = true, .cursor.state.blink_state = false, .cursor.state.blinking_mode = true, .cursor.symbol = 143};
