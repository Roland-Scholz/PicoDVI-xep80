...
void conio_reset(char default_cursor_symbol)
{
  ...
  printf("fontid: %d\n cursor.symbol: %02X \n", config.font_id, conio_config.cursor.symbol);
}
...
void clrscr()
{ // standard definition for clear screen
  char *sl;

  for (int r = 0; r < ROWS; r++)
  {
    // tighter method, as too much of a delay here can cause dropped characters
    sl = (char *)&ptr[r]->slot[0];
    
    memset(sl, 32, COLUMNS);
    /*
...
void clear_screen_from_cursor()
{
  ...
  for (int r = conio_config.cursor.pos.y + 1; r < ROWS; r++)
  {
    ...
    {
      put_char(32, c, r); // todo: should use the new method in clear_entire_screen
    }
  }
}
...
void insert_line()
{
  ...
  for (int i = 0; i < COLUMNS; i++)
  {
    ptr[conio_config.cursor.pos.y]->slot[i] = 32;
...
void clear_line_from_cursor()
{
  ...
  memset(sl, 32, COLUMNS - conio_config.cursor.pos.x);
...
void clear_line_to_cursor()
{
  ...
  memset(sl, 32, conio_config.cursor.pos.x);
...
void print_cursor()
{
  ...
  //printf("__chr_under_csr: %02X, __inv_under_csr: %02X, __blk_under_csr: %02X \n", __chr_under_csr, __inv_under_csr, __blk_under_csr);
  //printf("visible: %d, blink_mode: %d, blink_state: %d\n, conio_config.cursor.state.visible, conio_config.cursor.state.blinking_mode, conio_config.cursor.state.blink_state");

  if (conio_config.cursor.state.visible == false || (conio_config.cursor.state.blinking_mode && conio_config.cursor.state.blink_state))
    return;
...
void wrap_constrain_cursor_values()
{
  if (conio_config.cursor.pos.x >= COLUMNS)
  {
    ...
    if (conio_config.cursor.pos.y == VISIBLEROWS - 1)
    { // visiblerows is the count, csr is zero based
      shuffle_down();
    }
    else
    {
...
void constrain_cursor_values()
{
  if (conio_config.cursor.pos.x < 0)
    conio_config.cursor.pos.x = 0;
  if (conio_config.cursor.pos.x >= COLUMNS)
    conio_config.cursor.pos.x = COLUMNS - 1;
  if (conio_config.cursor.pos.y < 0)
    conio_config.cursor.pos.y = 0;
  if (conio_config.cursor.pos.y >= VISIBLEROWS)
    conio_config.cursor.pos.y = VISIBLEROWS - 1;
}
...
void cursor_visible(bool v)
{
  ...
  printf("conio_config.cursor.state.visible: %d\n", conio_config.cursor.state.visible);
  if (v == true)
    print_cursor();
  else
    clear_cursor();
